if(window. location.origin == 'http://localhost'){
    var url = window. location.origin + '/seas_prueba/controles/';
}else{
    var url = window. location.origin + '/controles/';
}
...
$(document).ready(function () {
    ...
    $('#calendario-legal-check').click(function () {
    ...
    $('#disponible-legal-check').click(function () {
    ...
    $('#nodisponible-legal-check').click(function () {
        var usuario = $("#s_login").val();
        var servicio = (this.getAttribute("servicio"));
        var id_Simulacion = $('#id_simulacion').val();
        var cedula = $('#cedula').val();
    ...
    $("#nodisponible-legal-check").mouseover(function () { alerta_error('Centrales Legales', 'Actualmente no tiene disponible una consulta de este cliente'); });
    $("#calendario-legal-check").mouseover(function () { alerta_info('Centrales Legales', 'Tiene mas de 30 dias la ultima consulta de este cliente'); });
    $("#disponible-legal-check").mouseover(function () { alerta_success('Centrales Legales', 'Tiene disponible una consulta reciente de este cliente'); });
    ...
    function verificar_disponibilidad(id_simulacion,cedula) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.LEGALCHECK=="0") {
                    $('#disponible-legal-check').hide();
                    $('#calendario-legal-check').hide();
                    $('#nodisponible-legal-check').show();
                }else{   
                    if (response.LEGALCHECK.fecha_vence == "reconsulta") {
                        $('#disponible-legal-check').attr("name",response.LEGALCHECK.pdf_respuesta2);
                        $('#disponible-legal-check').attr("id_consulta",response.LEGALCHECK.id_Consulta);
                        $('#disponible-legal-check').show();
                        $('#calendario-legal-check').hide();
                        $('#nodisponible-legal-check').hide();
                    } else {
                        $('#calendario-legal-check').show();
                        $('#nodisponible-legal-check').hide();
                        $('#disponible-legal-check').hide();
                    }
                }
