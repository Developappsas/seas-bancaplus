<?php
  ...
  /*$query = "SELECT c.municipio as municipio_expedicion,c.departamento as departamento_expedicion,
    ...
    b.nro_libranza,b.plazo,b.valor_credito,b.tasa_interes, TRUNCATE(b.tasa_interes, 2) AS tasa_interes_2decimal, k.nombre as oficina,j.nombre as nombre_comercial,
    j.apellido as apellido_comercial,j.cedula as cedula_comercial, l.firma_experian, l.pagare_deceval, b.pagaduria, a.peso, a.estatura
    FROM solicitud a
    ...
    LEFT JOIN formulario_digital l ON l.id_simulacion = b.id_simulacion
  ...
  $query = "SELECT c.municipio as municipio_expedicion,c.departamento as departamento_expedicion,
    ...
    h.municipio as municipio_familiar,h.departamento as departamento_familiar,
    i.municipio as municipio_personal,i.departamento as departamento_personal,
    a.*, b.formato_digital, b.opcion_credito, b.opcion_cuota_cli,b.opcion_cuota_ccc,b.opcion_cuota_cmp,b.opcion_cuota_cso,
    CONCAT(aa.nro_libranza, aa.consecutivo) as nro_libranza, aa.nro_afiliacion, aa.cuotas as plazo, aa.valor_cuota, b.tasa_interes, ROUND(b.tasa_interes, 2) AS tasa_interes_2decimal, k.nombre as oficina,j.nombre as nombre_comercial, b.id_simulacion,
    j.apellido as apellido_comercial,j.cedula as cedula_comercial, l.firma_experian, l.pagare_deceval, b.pagaduria, a.peso, a.estatura
    FROM incorporaciones_parciales aa
   left JOIN solicitud a ON a.id_simulacion = aa.id_simulacion    
    LEFT JOIN simulaciones b ON aa.id_simulacion=b.id_simulacion 
    LEFT JOIN ciudades c ON c.cod_municipio=a.lugar_expedicion
    LEFT JOIN ciudades d ON d.cod_municipio=a.lugar_nacimiento
    LEFT JOIN ciudades e ON e.cod_municipio=a.ciudad
    LEFT JOIN ciudades f ON f.cod_municipio=a.conyugue_lugar_expedicion
    ...
    LEFT JOIN usuarios j ON j.id_usuario=b.id_comercial
    LEFT JOIN oficinas k ON k.id_oficina=b.id_oficina 
    LEFT JOIN formulario_digital l ON l.id_simulacion = aa.id_simulacion
    WHERE aa.id = '".$_GET["id"]."'";
  ...
  $int_cuota_mensual = $fila["valor_cuota"];
  $int_cuota_descuento = $fila["valor_cuota"];
  $int_formato_digital = $fila["formato_digital"];

  $int_pagaduria = $fila["pagaduria"];

  $md5_firma_expirian=$fila["firma_experian"];
  $int_pagare_deceval = $fila["pagare_deceval"];

  $int_cantidad_cuotas_descuento=$fila["plazo"];
  $int_tasa_interes=$fila["tasa_interes"];
  $int_tasa_interes_2decimal=$fila["tasa_interes_2decimal"];
  $int_total_pagar =  intval($int_cantidad_cuotas_descuento)*intval($int_cuota_descuento);
  ...
  $str_cedula_comercial=$fila["cedula_comercial"];
  ...
  if ($fila["tipo_documento"]=="CEDULA") { $str_tipoidentificacion_solicitante = 'CC';}
  else if ($fila["tipo_documento"]=="REGISTRO CIVIL") { $str_tipoidentificacion_solicitante = 'RC';}
  else if ($fila["tipo_documento"]=="TARJETA DE IDENTIDAD") { $str_tipoidentificacion_solicitante = 'TI';}
  else if ($fila["tipo_documento"]=="CEDULA DE EXTRANJERIA") { $str_tipoidentificacion_solicitante = 'CE';}
  ...
  if ($fila["ocupacion"]==1) { $str_ocupacion_solicitante="EMPLEADO";}
  
  else if ($fila["ocupacion"]==3) { $str_ocupacion_solicitante="INDEPENDIENTE";}
  else if ($fila["ocupacion"]==4) { $str_ocupacion_solicitante="AMA DE CASA";}
  else if ($fila["ocupacion"]==5) { $str_ocupacion_solicitante="PENSIONADO";}
  else if ($fila["ocupacion"]==6) { $str_ocupacion_solicitante="ESTUDIANTE";}
  else if ($fila["ocupacion"]==7) { $str_ocupacion_solicitante="RENTISTA CAPITAL";}
  ...
  $str_estatura = $fila["estatura"];
  $str_peso = $fila["peso"];
  
  if ($fila["nivel_estudios"]=="PRIMARIA") { $str_nivel_estudios = "Primaria";}
  else if ($fila["nivel_estudios"]=="BACHILLER") { $str_nivel_estudios = "Bachiller";}
  else if ($fila["nivel_estudios"]=="TECNICO") { $str_nivel_estudios = "Tecnico";}
  else if ($fila["nivel_estudios"]=="TECNOLOGO") { $str_nivel_estudios = "Tecnologo";}
  else if ($fila["nivel_estudios"]=="UNIVERSITARIO") { $str_nivel_estudios = "Universitario";}
  else if ($fila["nivel_estudios"]=="ESPECIALIZACION") { $str_nivel_estudios = "Especializacion";}
  else if ($fila["nivel_estudios"]=="MAESTRIA") { $str_nivel_estudios = "Maestria";}
  else if ($fila["nivel_estudios"]=="DOCTORADO") { $str_nivel_estudios = "Doctorado";}
  ...
  $rowBen = 1;
  $queryBen = sqlsrv_query($link, "SELECT a.*, b.descripcion AS parentesco FROM solicitud_beneficiarios_seguro a
    LEFT JOIN parentescos b ON a.id_parentesco = b.id 
    WHERE a.id_simulacion = ".$fila['id_simulacion']." ORDER BY a.id");

  if(sqlsrv_num_rows($queryBen)>0){
    while ($filaBen = sqlsrv_fetch_array($queryBen)){ 

      $nombre = $filaBen["primer_nombre"]." ".$filaBen["segundo_nombre"]." ".$filaBen["primer_apellido"]." ".$filaBen["segundo_apellido"];

      if($filaBen["porcentaje"] != ''){
        $filaBen["porcentaje"] = $filaBen["porcentaje"].' %';
      }

      switch($rowBen){
        
        case 1:
          $str_nombreben1_titulo_gratuito=$nombre;
          $str_parenben1_titulo_gratuito=$filaBen["parentesco"];
          $str_porcben1_titulo_gratuito=$filaBen["porcentaje"];
          break;

        case 2:
          $str_nombreben2_titulo_gratuito=$nombre;
          $str_parenben2_titulo_gratuito=$filaBen["parentesco"];
          $str_porcben2_titulo_gratuito=$filaBen["porcentaje"];
          break;

        case 3:
          $str_nombreben3_titulo_gratuito=$nombre;
          $str_parenben3_titulo_gratuito=$filaBen["parentesco"];
          $str_porcben3_titulo_gratuito=$filaBen["porcentaje"];
          break;

        case 4:
          $str_nombreben4_titulo_gratuito=$nombre;
          $str_parenben4_titulo_gratuito=$filaBen["parentesco"];
          $str_porcben4_titulo_gratuito=$filaBen["porcentaje"];
          break;
      }
  ...
  $str_ciudad_diligenciamiento2=$fila["municipio_residencia"];
  //jairo
  ...
  $int_condicion1_declaracion_asegurabilidad = "";
  $int_condicion2_declaracion_asegurabilidad = "";
  $int_condicion3_declaracion_asegurabilidad = "";
  $int_condicion4_declaracion_asegurabilidad = "";
  $int_condicion5_declaracion_asegurabilidad = "";
  $int_condicion6_declaracion_asegurabilidad = "";
  $int_condicion7_declaracion_asegurabilidad = "";
  $int_condicion8_declaracion_asegurabilidad = "";

  $str_nombre_enfermedad_declaracion_asegurabilidad="";
  $str_tratamientos_medicos_declaracion_asegurabilidad="";
  $str_fecha_diagnostico_declaracion_asegurabilidad="";
  $str_secuelas_declaracion_asegurabilidad="";
  $str_tratamiento_actual_declaracion_asegurabilidad="";
  $str_observaciones_declaracion_asegurabilidad="";

  $int_tipo_novedad_reportar=1;

  $queryPreg = sqlsrv_query($link, "SELECT b.descripcion, a.respuesta, b.id FROM preguntas_seguro b LEFT JOIN solicitud_preguntas_seguro a ON a.id_pregunta = b.id AND a.id_simulacion = ".$fila['id_simulacion']." ORDER BY b.id", array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
  
  if(sqlsrv_num_rows($queryPreg)>0){
    while ($filaPreg = sqlsrv_fetch_array($queryPreg)){
      
      switch (intval($filaPreg["id"])) {
        case 1:
          $int_condicion1_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 2:
          $int_condicion2_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 3:
          $int_condicion3_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 4:
          $int_condicion4_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 5:
          $int_condicion5_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 6:
          $int_condicion6_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 7:
          $int_condicion7_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 8:
          $int_condicion8_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 9:
          $str_nombre_enfermedad_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 10:
          $str_fecha_diagnostico_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 11:
          $str_tratamientos_medicos_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 12:
          $str_secuelas_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 13:
          $str_tratamiento_actual_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;

        case 14:
          $str_observaciones_declaracion_asegurabilidad = $filaPreg["respuesta"];
          break;
      }
    }
  }
  
  $str_cuota_descuento_en_letra = utf8_decode(CifrasEnLetras::convertirNumeroEnLetras(str_replace(".",",",$int_cuota_mensual)));
  $int_valor_total_deuda=$int_total_pagar;
  $str_valor_total_prestamo = utf8_decode(CifrasEnLetras::convertirNumeroEnLetras(str_replace(".",",",$int_total_pagar)));
  ...
  $int_numero_afiliacion=$fila["nro_afiliacion"];
  ...
  if ($fila["conyugue_tipo_documento"]=="CEDULA") { $str_tipo_documento_conyuge = 'CC';}
  else if ($fila["conyugue_tipo_documento"]=="REGISTRO CIVIL") { $str_tipo_documento_conyuge = 'RC';}
  else if ($fila["conyugue_tipo_documento"]=="TARJETA DE IDENTIDAD") { $str_tipo_documento_conyuge = 'TI';}
  else if ($fila["conyugue_tipo_documento"]=="CEDULA DE EXTRANJERIA") { $str_tipo_documento_conyuge = 'CE';}
  ...
  $str_ano_fecha_nacimiento_conyuge=substr($str_fecha_nacimiento_conyuge, 0, 1).substr($str_fecha_nacimiento_conyuge, 1, 1).substr($str_fecha_nacimiento_conyuge, 2, 1).substr($str_fecha_nacimiento_conyuge, 3, 1);
  $str_mes_fecha_nacimiento_conyuge=substr($str_fecha_nacimiento_conyuge, 5, 1).substr($str_fecha_nacimiento_conyuge, 6, 1);
  $str_dia_fecha_nacimiento_conyuge=substr($str_fecha_nacimiento_conyuge, 8, 1).substr($str_fecha_nacimiento_conyuge, 9, 1);
  ...
  if ($fila["conyugue_dependencia"]=="SI") { 
    $str_dependencia_economica_conyuge = 's';
  }
  else if ($fila["conyugue_dependencia"]=="NO") { 
    $str_dependencia_economica_conyuge = 'n';
  }else{
    $str_dependencia_economica_conyuge = 'x';
  }
  ...
  if ($fila["declara_renta"]=="SI") { 
    $str_declara_renta = 's';
  }
  else if ($fila["declara_renta"]=="NO") { 
    $str_declara_renta = 'n';
  }else{
    $str_declara_renta = 'x';
  }
  
  if ($fila["funcionario_publico"]=="SI") { 
    $str_impacto_social_politica = 's';
  }
  else if ($fila["funcionario_publico"]=="NO") { 
    $str_impacto_social_politica = 'n';
  }else{
    $str_impacto_social_politica = 'x';
  }

  if ($fila["recursos_publicos"]=="SI") { 
    $str_maneja_recursos_publicos = 's';
  }
  else if ($fila["recursos_publicos"]=="NO") { 
    $str_maneja_recursos_publicos = 'n';
  }else{
    $str_maneja_recursos_publicos = 'x';
  }
  
  if ($fila["personaje_publico"]=="SI") { 
    $str_personaje_publico = true;
  }
  else if ($fila["personaje_publico"]=="NO") { 
    $str_personaje_publico = false;
  }
  ...
  if (in_array("EXPORTACION", $tipo_transaccion2)) { $str_operacion_exterior_exportacion = true;}else{$str_operacion_exterior_exportacion = false;}

  if (in_array("IMPORTACION", $tipo_transaccion2)) { $str_operacion_exterior_importacion = true;}else{$str_operacion_exterior_importacion = false;}
  
  if (in_array("INVERSION", $tipo_transaccion2)) { $str_operacion_exterior_inversiones = true;}else{$str_operacion_exterior_inversiones = false;}

  if (in_array("PRESTAMO EN MONEDA EXTRANJERA", $tipo_transaccion2)) { $str_operacion_exterior_prestamo = true;}else{$str_operacion_exterior_prestamo = false;}

  if (in_array("OTRA", $tipo_transaccion2)) { $str_operacion_exterior_otra = true;}else{$str_operacion_exterior_otra = false;}
  ...
  if ($fila["apoderado_tipo_documento"]=="CEDULA") { $str_apoderado_tipo_identificacion = 'CC';}
  else if ($fila["apoderado_tipo_documento"]=="REGISTRO CIVIL") { $str_apoderado_tipo_identificacion = 'RC';}
  else if ($fila["apoderado_tipo_documento"]=="TARJETA DE IDENTIDAD") { $str_apoderado_tipo_identificacion = 'TI';}
  else if ($fila["apoderado_tipo_documento"]=="CEDULA DE EXTRANJERIA") { $str_apoderado_tipo_identificacion = 'CE';}
