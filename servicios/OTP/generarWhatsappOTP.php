    ...
    if ($celular && $cedula){
        ...
        try {
            ...
            if ($data2->statusCode == 200) {
                ...
                $response = array("code" => "200", "mensaje" => "Se ejecutó correctamente la generación de codigo OTP por WhatsApp", "id" => $id_formulario);
            } else {
                header("HTTP/2.0 200 OK");
                $response = array("code" => "404", "mensaje" => "No Se Obtuvo Respuesta 2 del servidor", "data" => $data2);
            }
        } catch (\Throwable $e) {
            header("HTTP/2.0 200 OK");
            $response = array("code" => "404", "mensaje" => "Hubo un error al enviar el codigo por WhatsApp");
        }
    } else {
        header("HTTP/2.0 200 OK");
