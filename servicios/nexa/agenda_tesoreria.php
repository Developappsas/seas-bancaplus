...
/*		if (campofe.value == "") {
		...
		if (campofe.value != "" && campofv.value != "") {
			if (diffDate(campofe.value, campofv.value) < 0) {
				alert("La fecha de entrega no puede ser mayor que la fecha de vencimiento. Gesti�n no guardada");
				return false;
			}
			else {
				submit();
...
<tr>
	<td class="titulo"><center><b>Gesti&oacute;n Certificaciones</b><br><br></center></td>
</tr>
</table>
...
?>
	...
	<td valign="bottom">C&eacute;dula/Nombre<br><input type="text" name="descripcion_busqueda" onBlur="ReplaceComilla(this)" size="19" maxlength="50"></td>
<?php
...
$queryDB = "SELECT of.nombre as nombre_oficina,un.nombre as unidad_negocio,ag.*, si.cedula, si.nombre, si.pagaduria, us.nombre as nombre_comercial, us.apellido, et.nombre as nombre_etapa, se.nombre as nombre_subestado from agenda ag INNER JOIN simulaciones si ON ag.id_simulacion = si.id_simulacion INNER JOIN unidades_negocio un ON si.id_unidad_negocio = un.id_unidad INNER JOIN pagadurias pa ON si.pagaduria = pa.nombre INNER JOIN usuarios us ON si.id_comercial = us.id_usuario LEFT JOIN subestados se ON si.id_subestado = se.id_subestado LEFT JOIN etapas_subestados es ON se.id_subestado = es.id_subestado LEFT JOIN etapas et ON es.id_etapa = et.id_etapa LEFT JOIN oficinas of ON of.id_oficina=si.id_oficina where si.estado IN ('ING', 'EST') AND si.id_subestado IS NOT NULL AND si.id_subestado NOT IN (".$subestados_sin_concretar.") AND si.id_subestado IN (".$subestados_agenda.")";
