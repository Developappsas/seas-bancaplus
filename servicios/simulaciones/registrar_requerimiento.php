<?php
    ...
    if(isset($_POST['id_simulacion']) && isset($_POST['reqexcepReq']) && isset($_POST['observacionReq']) && isset($_POST['idTipoReq']) && isset($_POST['idAreaReq'])){
        ...
        if(sqlsrv_query($link, "INSERT INTO simulaciones_observaciones (id_simulacion, observacion, usuario_creacion, fecha_creacion) VALUES ('".$id_simulacion."', '[".$reqexcep."] ".utf8_encode($observacion)."', '".$_SESSION["S_LOGIN"]."', NOW())")){
            ...
            if(sqlsrv_query($link, "INSERT INTO req_excep (id_simulacion, reqexcep, id_tipo, id_area, fecha_vencimiento, observacion, id_observacion_pregunta, estado, usuario_creacion, fecha_creacion) VALUES ('".$id_simulacion."', '".$reqexcep."', '".$id_tipo."', '".$id_area."', ".$fecha_vencimiento.", '".utf8_encode($observacion)."', '".$id_observacion_pregunta."', 'PENDIENTE', '".$_SESSION["S_LOGIN"]."', GETDATE())")){
                ...
                $id = sqlsrv_quey($link, "SELECT SCOPE_IDENTITY() as id");
                ...
                if(sqlsrv_query($link, "UPDATE simulaciones SET id_subestado = ".$id_subestado." WHERE id_simulacion  = '".$id_simulacion."'")){
                    sqlsrv_query($link, "INSERT INTO simulaciones_subestados (id_simulacion, id_subestado, usuario_creacion, fecha_creacion) values ('".$id_simulacion."', ".$id_subestado.", 'system', NOW())");//6.1 RECHAZADO PDTE CERTIFICADO BANCARIO.
                }
                ...
                if (sqlsrv_num_rows($querySimulacionesFdc)>0){
                    ...
                    $consultarUsuarioNuevo=sqlsrv_query($link, "SELECT a.id_usuario, a.nombre, a.cantidad_asignado, b.cantidad_terminado, (a.cantidad_asignado+b.cantidad_terminado) AS cantidad_total
                      ...
                      (SELECT a.id_usuario,a.nombre,a.apellido,COUNT(b.id) AS cantidad_terminado
                        FROM usuarios a 
                        LEFT JOIN  (SELECT * FROM simulaciones_fdc WHERE estado=4 AND id_subestado not in (28,53) AND DATE_FORMAT(fecha_creacion,'%Y-%m-%d') = CURRENT_DATE()) b 
                ...
                if (strcmp($_FILES["archivoReq"]["name"], "")){
                    ...
                    sqlsrv_query($link, "INSERT INTO req_excep_adjuntos (id_reqexcep, descripcion, nombre_original, nombre_grabado, usuario_creacion, fecha_creacion) VALUES ('".$id_reqexcep."', '".utf8_encode($_REQUEST["descripcion"])."', '".reemplazar_caracteres_no_utf($_FILES["archivoReq"]["name"])."', '".$uniqueID."_".reemplazar_caracteres_no_utf($_FILES["archivoReq"]["name"])."', '".$_SESSION["S_LOGIN"]."', NOW())");
