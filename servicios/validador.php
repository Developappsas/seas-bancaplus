<?php
    ...
    function validador(){
        ...
        $query_token = ("SELECT * FROM token_proveedores a LEFT JOIN  proveedores_api b on a.id_proveedor=b.id_proveedor LEFT JOIN api c ON c.id_api=b.id_api WHERE c.url='$servicio' and a.estado = 0 AND a.token = '$token'");
        $ejecucion = sqlsrv_query($link,$query_token, array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
        if ( sqlsrv_num_rows($ejecucion) > 0) 
        {
            # code...
            
            $query_update = ("UPDATE token_proveedores SET estado = 1 WHERE token = '$token'");
