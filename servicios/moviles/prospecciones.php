<?php
    ...
    switch ($proceso) {
        case 'Lista Prospecciones':
            ...
            if ($ejecutar_query_select_simulaciones) {
                if (sqlsrv_num_rows($ejecutar_query_select_simulaciones) > 0 ) {
                    ...
                    $respuesta = array("estado" => "404", "descripcion" => "Not Found", "mensaje" => "Solicitud no encontrada", "fecha" => date("Y-m-d"), 'Proceso' => $proceso,"consulta" => $query_select_simulaciones);
                }

            }else {    
                $respuesta = array("estado" => "500", "descripcion" => "Not Found", "mensaje" => "Solicitud no encontrada", "fecha" => date("Y-m-d"), 'Proceso' => $proceso, "error" => "-->".mysqli_error($link)."<--" );
            }
        ...
        case 'Nueva':
            ...
            $ejecutar_query_insert_simulacion = mysqli_query($link, $query_insert_simulacion);
            if ($ejecutar_query_insert_simulacion) {
                while ( $resultado = mysqli_fetch_assoc($ejecutar_query_insert_simulacion) ) {
