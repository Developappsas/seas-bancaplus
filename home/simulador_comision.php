...
if(mysqli_num_rows($query_oficina) > 0){
	$datos_ofi = mysqli_fetch_array($query_oficina);
		...
		<tr>
			...
			<td>
				<select id="tipo_tasa_comision" name="tipo_tasa_comision"  style="width:195px; background-color:#EAF1DD;">
					...
					//Tasa Comisones
					if ($id_unidad_negocio == 4 || $id_unidad_negocio == 11 || $id_unidad_negocio == 14 || $id_unidad_negocio == 19) {
						$id_unidad_negocio_tasa_comision = 4; //Fianti
					}

					if ($id_unidad_negocio == 6 || $id_unidad_negocio == 15) {
						$id_unidad_negocio_tasa_comision = 6; //Atraccion
					}

					if ($id_unidad_negocio == 2 || $id_unidad_negocio == 12 || $id_unidad_negocio == 16) {
						$id_unidad_negocio_tasa_comision = 2; //Salvamento
					}

					if ($id_unidad_negocio == 1 || $id_unidad_negocio == 10 || $id_unidad_negocio == 17) {
						$id_unidad_negocio_tasa_comision = 1; //Salvamento
					}

					$queryTasaC = "SELECT a.marca_unidad_negocio, a.id_tasa_comision, a.id_unidad_negocio, c.nombre AS unidad_negocio, a.tasa AS tasa_interes, a.kp_plus, a.id_tipo, a.fecha_inicio, if(a.fecha_fin IS NULL, '',a.fecha_fin) AS fecha_fin, a.vigente FROM tasas_comisiones a LEFT JOIN unidades_negocio c ON c.id_unidad = a.id_unidad_negocio WHERE a.id_unidad_negocio = $id_unidad_negocio_tasa_comision AND a.tasa = $tasa_interes AND a.kp_plus = 1 AND NOW() >= a.fecha_inicio AND (a.fecha_fin IN('0000-00-00') OR a.fecha_fin IS NULL OR a.fecha_fin <= NOW());";

					$queryTasaC = "SELECT a.marca_unidad_negocio, a.id_tasa_comision, a.id_unidad_negocio, c.nombre AS unidad_negocio, a.tasa AS tasa_interes, a.kp_plus, a.id_tipo, a.fecha_inicio, if(a.fecha_fin IS NULL, '',a.fecha_fin) AS fecha_fin, a.vigente FROM tasas_comisiones a LEFT JOIN unidades_negocio c ON c.id_unidad = a.id_unidad_negocio WHERE (DATE_FORMAT(NOW(), '%Y-%m-%d') >= a.fecha_inicio AND DATE_FORMAT(NOW(), '%Y-%m-%d') <= a.fecha_fin) OR a.vigente = 1;";*/
					...
					while ($fTC = mysqli_fetch_array($rsTC)){ 
					...
					$id_tasa_comision = $fila['id_tasa_comision'];
					?>							
				</select>
			</td>
		</tr>
...
<tr><td colspan="3" align="center">
	...
	<input type="hidden" name="id_analista_riesgo_operativoh" value="<?=$id_analista_riesgo_operativoh; ?>">
	<input type="hidden" name="id_tasa_comisionh" id="id_tasa_comisionh" value="<?=$id_tasa_comision; ?>">
