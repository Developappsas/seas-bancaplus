...
while ($fila = sqlsrv_fetch_array($rs, SQLSRV_FETCH_ASSOC)){   
	...
	$val=0;
		...
		$consultarUltimoFechaTesoreria=sqlsrv_query($link,"SELECT top 1 * FROM simulaciones_subestados WHERE id_simulacion='".$fila["id_simulacion"]."' AND id_subestado=46 ORDER BY id_simulacionsubestado DESC ");
		...
		if ($val==0){
			$consultarUltimoFechaTesoreria=sqlsrv_query($link,"SELECT top 1 * FROM simulaciones_subestados WHERE id_simulacion='".$fila["id_simulacion"]."' AND id_subestado=48 ORDER BY id_simulacionsubestado DESC ");
			if (sqlsrv_num_rows($consultarUltimoFechaTesoreria)>0){
				$resUltimoFechaTesoreria=sqlsrv_fetch_array($consultarUltimoFechaTesoreria);
				$fecha_ultima_tesoreria=$resUltimoFechaTesoreria["fecha_creacion"];
				$val=1;
