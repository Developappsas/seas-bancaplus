...
<?php
	...
	if ($_REQUEST["descripcion_busqueda"])
	{
		...
		$queryDB_count .= " AND (si.cedula = '".$descripcion_busqueda."' OR UPPER(si.nombre) like '%".(strtoupper($descripcion_busqueda))."%')";
		
		$queryDB_suma .= " AND (si.cedula = '".$descripcion_busqueda."' OR UPPER(si.nombre) like '%".(strtoupper($descripcion_busqueda))."%')";
	}
			...
			<table class="tab3" id="tablaFDC">
				...
				<?php
					...
					while ($fila = sqlsrv_fetch_array($rs, SQLSRV_FETCH_ASSOC))
					{
						...
						$consultarTiempoRadicado=sqlsrv_query("SELECT * FROM simulaciones_fdc WHERE id=(SELECT max(id) FROM simulaciones_fdc where estado=5 and id_simulacion='".$fila["id_simulacion"]."')",$link);
				...
				<td><?php echo $fila["id_simulacion"];?></td>
					...
					<td<?php if (($_SESSION["S_TIPO"] == "ADMINISTRADOR" || $_SESSION["S_TIPO"] == "OPERACIONES" || $_SESSION["S_SUBTIPO"] == "COORD_CREDITO" || $_SESSION["S_SUBTIPO"] == "ANALISTA_CREDITO") && $fila["empleado_manual"]) { ?> style="color:#FF0000"<?php } ?>><?php echo ($fila["nombre"]) ?></td>
					<td><?php echo ($fila["pagaduria"]) ?></td>
					<?php if ($_SESSION["S_TIPO"] != "COMERCIAL") { ?><td><?php echo ($fila["nombre_comercial"]." ".$fila["apellido"]) ?></td><?php } ?>
					<!--<th>SE AGREGA COLUMNA TIPO COMERCIAL EN TABLA PILOTO FDC. ING JAIRO ZAPATA FECHA 23/11/2021</th>-->	<?php if ($_SESSION["S_TIPO"] != "COMERCIAL") { ?><td><?php echo ($fila["tipo_comercial2"]) ?></td><?php } ?>
