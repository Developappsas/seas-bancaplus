...
<tr>
	<td class="titulo"><center><b>Documentos Cierre Ventas</b><br><br></center></td>
</tr>
</table>
<form name=formato method=post action="documentoscierre_crear.php" onSubmit="return chequeo_forma()">
<table>
<tr>
<td>
...
<tr>
	...
	<td valign="bottom">&nbsp;<br><input type="submit" value="Crear Documento"></td>
</tr>
</table>
</div>
</td>
...
<form name="formato2" method="post" action="documentoscierre.php">
<table>
<tr>
<td>
...
$queryDB = "select * from documentos_cierre where id_documentocierre IS NOT NULL";

$queryDB_count = "select COUNT(*) as c from documentos_cierre where id_documentocierre IS NOT NULL";
	...
	if ($cuantos > $x_en_x)
	{
		...
		while ($final < $cuantos)
		{
			...
			if ($link_page != $_REQUEST["page"])
				{
				echo " <a href=\"documentoscierre.php?descripcion_busqueda=".$descripcion_busqueda."&page=$link_page\">$i</a>";
			}
			else
			{
				echo " ".$i;
		...
		if ($_REQUEST["page"] != $link_page)
		{
			...
			echo " <a href=\"documentoscierre.php?descripcion_busqueda=".$descripcion_busqueda."&page=".$siguiente_page."\">Siguiente</a></p></td></tr>";
		}
...
?>
<form name="formato3" method="post" action="documentoscierre_actualizar.php">
...
<tr <?php echo $tr_class ?>>
	<td><input type="text" name="n<?php echo $fila["id_documentocierre"] ?>" value="<?php echo str_replace("\"", "&#34", utf8_decode($fila["nombre"])) ?>" maxlength="255" size="50"></td>
	<td align="center"><input type="checkbox" name="a<?php echo $fila["id_documentocierre"] ?>" value="1"<?php if ($fila["estado"]) { echo " checked"; } ?>></td>
	<td><input type=button value="Modificar" onClick="document.formato3.action.value='actualizar'; document.formato3.id.value='<?php echo $fila["id_documentocierre"] ?>'; modificar(document.formato3.n<?php echo $fila["id_documentocierre"] ?>)"></td>
	<td align=center><input type=checkbox name="b<?php echo $fila["id_documentocierre"] ?>" value="1"></td>
</tr>
<?php
