...
// header("Content-Disposition: attachment; filename=Coltefinanciera - Clientes " . $venta["nro_venta"] . ".csv");
// header("Pragma: no-cache");
// header("Expires: 0");

if (!$_REQUEST["ext"]) {
	$queryDB = "SELECT si.id_simulacion, so.nombre1, so.nombre2, so.apellido1, so.apellido2, so.sexo, FORMAT(si.fecha_nacimiento, 'dd/MM/yyyy') as fecha_nacimiento, so.lugar_nacimiento, so.tipo_documento, si.cedula, FORMAT(so.fecha_expedicion, 'dd/MM/yyyy') as fecha_expedicion, so.lugar_expedicion, so.estado_civil, so.personas_acargo, so.nivel_estudios, so.email, so.tipo_vivienda, so.direccion, so.residencia_barrio, so.ciudad, ci.municipio, ci.departamento, so.residencia_estrato, so.tel_residencia, so.celular, so.ocupacion, si.nivel_contratacion, so.nombre_empresa, si.pagaduria, so.direccion_trabajo, so.ciudad_trabajo, so.telefono_trabajo, so.tipo_contrato, FORMAT(so.fecha_vinculacion, 'dd/MM/yyyy') as fecha_vinculacion, so.cargo, si.total_ingresos, si.total_egresos, so.total_activos, so.total_pasivos, so.declara_renta, so.moneda_extranjera, si.plazo, si.valor_credito, si.puntaje_datacredito, si.retanqueo1_libranza, si.retanqueo1_valor, si.retanqueo2_libranza, si.retanqueo2_valor, si.retanqueo3_libranza, si.retanqueo3_valor, pp.nit as nit_pagaduria, pp.pa, si.nro_libranza from ventas_detalle vd INNER JOIN simulaciones si ON vd.id_simulacion = si.id_simulacion INNER JOIN pagadurias pa ON si.pagaduria = pa.nombre LEFT JOIN solicitud so ON so.id_simulacion = si.id_simulacion LEFT JOIN pagaduriaspa pp ON si.pagaduria = pp.pagaduria LEFT JOIN ciudades ci ON so.ciudad = ci.cod_municipio where vd.id_venta = '" . $_REQUEST["id_venta"] . "'";
...
while($fila = sqlsrv_fetch_array($rs, SQLSRV_FETCH_ASSOC)){
	...
	if (strlen(trim($fila["telefono_trabajo"])) < 7)
		...
		$telefono_oficina = is_numeric(substr(trim($fila["telefono_trabajo"]), 0, 10)) ? substr(trim($fila["telefono_trabajo"]), 0, 10): 
	 is_numeric(substr(trim($fila["celular"]), 0, 10)) ? substr(trim($fila["celular"]), 0, 10) : 0;
