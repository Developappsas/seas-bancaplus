...
if (!$_SESSION["S_LOGIN"] || !($_SESSION["S_TIPO"] == "ADMINISTRADOR" || $_SESSION["S_TIPO"] == "OPERACIONES" || $_SESSION["S_TIPO"] == "COMERCIAL" || $_SESSION["S_TIPO"] == "DIRECTOROFICINA" || $_SESSION["S_TIPO"] == "PROSPECCION" || $_SESSION["S_SUBTIPO"] == "ANALISTA_GEST_COM" || $_SESSION["S_SUBTIPO"] == "ANALISTA_REFERENCIA" || $_SESSION["S_SUBTIPO"] == "ANALISTA_CREDITO" || $_SESSION["S_SUBTIPO"] == "ANALISTA_TESORERIA" || $_SESSION["S_SUBTIPO"] == "ANALISTA_VEN_CARTERA" || $_SESSION["S_SUBTIPO"] == "COORD_PROSPECCION"))
{
    exit;
}

$queryDB = "SELECT us.freelance, us.outsourcing from simulaciones si INNER JOIN usuarios us ON si.id_comercial = us.id_usuario where si.id_simulacion = '".$_REQUEST["id_simulacion"]."'";

$simulacion_rs = sqlsrv_query($link, $queryDB, array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
//Prueba Query
if ($simulacion_rs == false) {
...
if ($simulacion["freelance"])
	$tipo_comercial = "FREELANCE";

if ($simulacion["outsourcing"])
	$tipo_comercial = "OUTSOURCING";

if (($_SESSION["S_TIPO"] == "COMERCIAL" || $_SESSION["S_TIPO"] == "DIRECTOROFICINA") && $tipo_comercial != "PLANTA")
{
    exit;
}
...
<?php
	...
	$queryDB = "SELECT so.*, ci1.municipio as nombre_lugar_expedicion, ci2.municipio as nombre_lugar_nacimiento, ci3.municipio as nombre_ciudad, ci4.municipio as nombre_ciudad_familiar, ci5.municipio as nombre_ciudad_personal from solicitud so INNER JOIN simulaciones si ON so.id_simulacion = si.id_simulacion INNER JOIN pagadurias pa ON si.pagaduria = pa.nombre left join ciudades ci1 ON ci1.cod_municipio = so.lugar_expedicion left join ciudades ci2 ON ci2.cod_municipio = so.lugar_nacimiento left join ciudades ci3 ON ci3.cod_municipio = so.ciudad left join ciudades ci4 ON ci4.cod_municipio = so.ciudad_familiar left join ciudades ci5 ON ci5.cod_municipio = so.ciudad_personal where so.id_simulacion = '".$_REQUEST["id_simulacion"]."'";
...
<tr><td colspan="6" class="tilenews3">CONOCIMIENTO<BR>DEL CLIENTE</td></tr>
           ...
           &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        ...
                        <td ><label class="admintable">Asesor:</label><b class="admintable"> <?php

    $queryDB = "SELECT  id_usuario,nombre, apellido from usuarios";

    $rz1 = sqlsrv_query($link, $queryDB, array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
    //Prueba Query
    if ($rz1 == false) {
        if( ($errors = sqlsrv_errors() ) != null) {
            foreach( $errors as $error ) {
    ...
    while ($fila = sqlsrv_fetch_array($rz1))
    {
        ...
        echo strtoupper("".utf8_decode($fila["nombre"])." ".utf8_decode($fila["apellido"])."")
        }
...
?></b></td>
            <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td ><label style="" class="admintable">Calificaci&oacute;n:</label><b class="admintable">  <?php echo strtoupper($fila1["calificacion"]) ?></b></td>
    ...
    <table border="0" cellspacing=3 cellpadding=0 width="850px">
<tr>
  <td colspan="6" style="width: 100%; text-align: center; background:#000000;" class="tilenews2">Instrucciones y documentos requeridos</td>

</tr>

      <tr>
                        <td><label  class="admintable">Fecha:</label><b class="admintable">  <?php echo strtoupper($fila1["fecha"]) ?></b></td>

                        <td colspan="3" class="admintable"><ol>
                                <li>Diligencie completamente la solicitud SIN TACHONES NI ENMENDADURAS</li>
                                <li>Escriba sus nombres y apellidos igual a como aparecen en el documento del cliente</li>
                                <li>Anexe dos fotocopias de la cedula AMPLIADA Al 150%</li>
                            </ol></td>
                    </tr>
  ...
  <tr>
                    ...
                    <tr>
                        ...
                        <td colspan="2"><label class="admintable">Nombre del conyuge:</label><b class="admintable"> <?php echo strtoupper($fila1["nombre_conyugue"]." ".$fila1["conyugue_nombre_2"]." ".$fila1["conyugue_apellido_1"]." ".$fila1["conyugue_apellido_2"])  ?></b></td>
  ...
  <tr>
                   ...
                   <table border="1" cellspacing=0 cellpadding=5 width="850px">
                    ...
                    <tr>
                        ...
                        <td colspan="2" class="admintable"><label class="admintable">Ciudad:</label> <b class="admintable"> <?php echo strtoupper($fila1["nombre_ciudad_familiar"]) ?></b></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td colspan="2" class="admintable"><label class="admintable">Ciudad:</label> <b class="admintable"> <?php echo strtoupper($fila1["nombre_ciudad_personal"]) ?></b></td>
                    </tr>
