...
sqlsrv_query($link, "delete from cazador where id_usuario = '0' ");

$parametros = sqlsrv_query($link, "select * from parametros where tipo = 'SIMULADOR' or tipo = 'CAZADOR' order by codigo");
...
$es_freelance = sqlsrv_query($link, "select * from usuarios where id_usuario = '".$_SESSION["S_IDUSUARIO"]."' and freelance = '1'");
...
//exit;
		...
		while($fila = sqlsrv_fetch_array($empleado))
		{
			...
			if ($plazo)
				{
					$rs_porcentaje_seguro = sqlsrv_query($link, "SELECT porcentaje from porcentajes_seguro where tipo_empleado = '".$tipo_empleado."' AND plazo = '".$plazo."'");
				...
				$embargos = sqlsrv_query($link, "select * from ".$prefijo_tablas."embargos where cedula = '".$fila["cedula"]."'");
				...
				$rechazos = sqlsrv_query($link, "select * from ".$prefijo_tablas."rechazos where cedula = '".$fila["cedula"]."'");
...
//exit;
				sqlsrv_query($link, "insert into cazador (cedula , nombre, pagaduria, institucion, cargo, grado, salario_basico, ingresos, egresos, neto_pagar, nivel_educativo, direccion, telefono, mail, fecha_nacimiento, nivel_contratacion, ciudad, estado_cargue, sexo, ciudad_labora, fecha_inicio_labor, desembolso_cliente, valor_credito1, valor_credito2, libre_inversion, decision, fecha_asignacion) VALUES ('".$fila["cedula"]."', '".$fila["nombre"]."', '".$fila["pagaduria"]."', '".$fila["institucion"]."', '".$fila["cargo"]."', '".$fila["grado"]."', '".$fila["salario_basico"]."', '".$fila["ingresos"]."', '".$fila["egresos"]."', '".$fila["neto_pagar"]."', '".$fila["nivel_educativo"]."', '".$fila["direccion"]."', '".$fila["telefono"]."', '".$fila["mail"]."', '".$fila["fecha_nacimiento"]."', '".$fila["nivel_contratacion"]."', '".$fila["ciudad"]."', '".$fila["estado_cargue"]."', '".$fila["sexo"]."', '".$fila["ciudad_labora"]."', '".$fila["fecha_inicio_labor"]."', '".$opcion_cuota_ccc."', '".$opcion_desembolso_ccc."', '".$valor_credito2."', '".$opcion_desembolso_cli."', '".$decision."', now())");
