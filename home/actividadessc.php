...
<table border="0" cellspacing=1 cellpadding=2>
	<tr>
		<td class="titulo">
			<center><b>Actividades/Solicitudes Servicio al Cliente</b><br><br></center>
		</td>
	</tr>
</table>
<form name=formato method=post action="actividadessc_crear.php" onSubmit="return chequeo_forma()">
	<table>
		<tr>
			<td>
				<div class="box1 clearfix">
					<table border="0" cellspacing=1 cellpadding=2>
						<tr>
							...
							<td valign="bottom">&nbsp;<br><input type="submit" value="Crear Actividad/Solicitud"></td>
						</tr>
					</table>
				</div>
			</td>
...
<form name="formato2" method="post" action="actividadessc.php">
	<table>
		<tr>
			<td>
...
$queryDB = "SELECT * from actividadessc where id_actividad IS NOT NULL";

$queryDB_count = "SELECT COUNT(*) as c from actividadessc where id_actividad IS NOT NULL";
	...
	if ($cuantos > $x_en_x) {
		...
		while ($final < $cuantos) {
			...
			if ($link_page != $_REQUEST["page"]) {
				echo " <a href=\"actividadessc.php?descripcion_busqueda=" . $descripcion_busqueda . "&page=$link_page\">$i</a>";
			} else {
				echo " " . $i;
			}
		...
		if ($_REQUEST["page"] != $link_page) {
			...
			echo " <a href=\"actividadessc.php?descripcion_busqueda=" . $descripcion_busqueda . "&page=" . $siguiente_page . "\">Siguiente</a></p></td></tr>";
		}
...
?>
	<form name="formato3" method="post" action="actividadessc_actualizar.php">
		...
		<table border="0" cellspacing=1 cellpadding=2 class="tab1">
			...
			?>
				<tr <?php echo $tr_class ?>>
					<td><input type="text" name="n<?php echo $fila["id_actividad"] ?>" value="<?php echo str_replace("\"", "&#34", utf8_decode($fila["nombre"])) ?>" maxlength="255" size="50"></td>
					<td align="center"><input type="checkbox" name="a<?php echo $fila["id_actividad"] ?>" value="1" <?php if ($fila["estado"]) {
																														echo " checked";
																													} ?>></td>
					<td><input type=button value="Modificar" onClick="document.formato3.action.value='actualizar'; document.formato3.id.value='<?php echo $fila["id_actividad"] ?>'; modificar(document.formato3.n<?php echo $fila["id_actividad"] ?>)"></td>
					<td align=center><input type=checkbox name="b<?php echo $fila["id_actividad"] ?>" value="1"></td>
				</tr>
			<?php
