...
$link = conectar_utf();
$upmax_rs = mysqli_query($link, "select valor from parametros where codigo IN ('UPMAX') order by codigo");

$fila1 = mysqli_fetch_assoc($upmax_rs);

$upmax = $fila1['valor'];

$queryDB = "select estado from simulaciones where id_simulacion = '" . $_REQUEST["id_simulacion"] . "'";

$simulacion_rs = mysqli_query($link, $queryDB);

$simulacion = mysqli_fetch_assoc($simulacion_rs);
		...
		while ($fila = mysqli_fetch_assoc($rs)) {
			if ($_REQUEST["chk" . $fila["id_adjunto"]] == "1") {
				if ($_REQUEST["action"] == "borrar" && $habilitar_borrar) {
					...
					$archivo = mysqli_query($link, "select id_tipo, nombre_grabado from adjuntos where id_adjunto = '" . $fila["id_adjunto"] . "'");
					$fila1 = mysqli_fetch_assoc($archivo);
					// if ($fila1["nombre_grabado"]){
					// 	delete_file("simulaciones", $_REQUEST["id_simulacion"] . "/adjuntos/" . $fila1["nombre_grabado"]);
					// }
					mysqli_query($link, "update tesoreria_cc set id_adjunto = NULL, usuario_modificacion = '". $_SESSION["S_LOGIN"] ."', fecha_modificacion = NOW() where id_adjunto = '" . $fila["id_adjunto"] . "'");
					mysqli_query($link, "update simulaciones_comprascartera set id_adjunto = NULL where id_adjunto = '".$fila["id_adjunto"]."'");
					$elimino = mysqli_query($link, "update adjuntos set eliminado = 1,  fecha_eliminacion = NOW(),  usuario_eliminacion ='".$_SESSION["S_LOGIN"]."' where id_adjunto = '".$fila['id_adjunto']."'");
				}
			}
		}
	}
...
<div id="adjuntos_virtuales" class="tabcontent">
	...
	$val2 = 0;
	$val3 = 0;
	$queryADO = mysqli_query($link, "SELECT id_simulacion FROM historial_tokens_verificacion_id f WHERE f.id = (SELECT MAX(id) FROM historial_tokens_verificacion_id WHERE id_simulacion = f.id_simulacion) AND f.id_simulacion = " . $_REQUEST["id_simulacion"] . " AND f.estado = 1 AND estado_respuesta IS NOT NULL AND estado_respuesta IN(2,14)");
	$val1 = mysqli_num_rows($queryADO);

	$queryFormularioDigital = mysqli_query($link, "SELECT b.id_unidad_negocio, un.id_empresa, a.* FROM formulario_digital a JOIN simulaciones b ON b.id_simulacion = a.id_simulacion LEFT JOIN unidades_negocio un ON b.id_unidad_negocio = un.id_unidad WHERE a.id_simulacion = '" . $_REQUEST["id_simulacion"] . "' AND b.formato_digital = 1");
	$val2 = mysqli_num_rows($queryFormularioDigital);

	$queryOTP = mysqli_query($link, "SELECT id_simulacion FROM historial_sms_otp f WHERE f.id = (SELECT MAX(id) FROM historial_sms_otp WHERE id_simulacion = f.id_simulacion) AND f.id_simulacion = '" . $_REQUEST["id_simulacion"] . "' AND f.estado = 1");
	$val3 = mysqli_num_rows($queryOTP);
	...
	if ($val1 <> 0 || $val2 <> 0 || $val3 <> 0) { ?>
		...
		<table border="0" cellspacing=1 cellpadding=2 class="tab2" id="tablaAdjuntosDigitales" name="tablaAdjuntosDigitales">
			...
			if ($val2 <> 0) {
				$empresa = "kredit";
				$datosFrmDigital = mysqli_fetch_assoc($queryFormularioDigital);
