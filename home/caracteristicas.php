...
<table border="0" cellspacing=1 cellpadding=2>
	<tr>
		<td class="titulo">
			<center><b>Caracter&iacute;sticas</b><br><br></center>
		</td>
	</tr>
</table>
<form name=formato method=post action="caracteristicas_crear.php" onSubmit="return chequeo_forma()">
	<table>
		<tr>
			<td>
				<div class="box1 clearfix">
					<table border="0" cellspacing=1 cellpadding=2>
						<tr>
							...
							<td valign="bottom">&nbsp;<br><input type="submit" value="Crear Caracter&iacute;stica"></td>
						</tr>
					</table>
				</div>
			</td>
...
<form name="formato2" method="post" action="caracteristicas.php">
	<table>
		<tr>
			<td>
...
$queryDB = "SELECT * from caracteristicas where id_caracteristica IS NOT NULL";

$queryDB_count = "SELECT COUNT(*) as c from caracteristicas where id_caracteristica IS NOT NULL";
	...
	if ($cuantos > $x_en_x) {
		...
		while ($final < $cuantos) {
			...
			if ($link_page != $_REQUEST["page"]) {
				echo " <a href=\"caracteristicas.php?descripcion_busqueda=" . $descripcion_busqueda . "&page=$link_page\">$i</a>";
			} else {
				echo " " . $i;
			}
		...
		if ($_REQUEST["page"] != $link_page) {
			...
			echo " <a href=\"caracteristicas.php?descripcion_busqueda=" . $descripcion_busqueda . "&page=" . $siguiente_page . "\">Siguiente</a></p></td></tr>";
		}
...
?>
	<form name="formato3" method="post" action="caracteristicas_actualizar.php">
		...
		<table border="0" cellspacing=1 cellpadding=2 class="tab1">
			...
			?>
				<tr <?php echo $tr_class ?>>
					<td><input type="text" name="n<?php echo $fila["id_caracteristica"] ?>" value="<?php echo str_replace("\"", "&#34", utf8_decode($fila["nombre"])) ?>" maxlength="255" size="50"></td>
					<td align="center"><input type="checkbox" name="a<?php echo $fila["id_caracteristica"] ?>" value="1" <?php if ($fila["estado"]) {
																																echo " checked";
																															} ?>></td>
					<td><input type=button value="Modificar" onClick="document.formato3.action.value='actualizar'; document.formato3.id.value='<?php echo $fila["id_caracteristica"] ?>'; modificar(document.formato3.n<?php echo $fila["id_caracteristica"] ?>)"></td>
					<td align=center><input type=checkbox name="b<?php echo $fila["id_caracteristica"] ?>" value="1"></td>
				</tr>
			<?php
