...
<tr>
	<td class="titulo"><center><b>Causales No Recaudo</b><br><br></center></td>
</tr>
</table>
<form name=formato method=post action="causalesnorecaudo_crear.php" onSubmit="return chequeo_forma()">
<table>
<tr>
<td>
...
<tr>
	<td valign="bottom">Tipo<br>
		<select name="id_tipo" style="width:200px">
			<option value=""></option>
<?php

$queryDB = "SELECT id_tipo, nombre from tipos_causalesnorecaudo where estado = '1' order by nombre";
...
<form name="formato2" method="post" action="causalesnorecaudo.php">
<table>
<tr>
<td>
...
$queryDB = "SELECT cnr.* from causales_norecaudo cnr INNER JOIN tipos_causalesnorecaudo tcr ON cnr.id_tipo = tcr.id_tipo where cnr.id_causal IS NOT NULL";

$queryDB_count = "SELECT COUNT(*) as c from causales_norecaudo cnr INNER JOIN tipos_causalesnorecaudo tcr ON cnr.id_tipo = tcr.id_tipo where cnr.id_causal IS NOT NULL";
...
if ($_REQUEST["descripcion_busqueda"])
{
	...
	$queryDB_count = $queryDB_count." AND UPPER(cnr.nombre) like '%".utf8_encode(strtoupper($descripcion_busqueda))."%'";
}

$queryDB .= " order by tcr.nombre, cnr.nombre DESC OFFSET ".$offset." ROWS";
	...
	if ($cuantos > $x_en_x)
	{
		...
		while ($final < $cuantos)
		{
			...
			if ($link_page != $_REQUEST["page"])
				{
				echo " <a href=\"causalesnorecaudo.php?descripcion_busqueda=".$descripcion_busqueda."&page=$link_page\">$i</a>";
			}
			else
			{
				echo " ".$i;
		...
		if ($_REQUEST["page"] != $link_page)
		{
			...
			echo " <a href=\"causalesnorecaudo.php?descripcion_busqueda=".$descripcion_busqueda."&page=".$siguiente_page."\">Siguiente</a></p></td></tr>";
		}
...
?>
<form name="formato3" method="post" action="causalesnorecaudo_actualizar.php">
...
<tr <?php echo $tr_class ?>>
    <td><select name="t<?php echo $fila["id_causal"] ?>" style="width:200px">
			<option value=""></option>
<?php

		$queryDB = "SELECT id_tipo, nombre from tipos_causalesnorecaudo where estado = '1' OR id_tipo = '".$fila["id_tipo"]."'";
		...
		while ($fila1 = sqlsrv_fetch_array($rs1, SQLSRV_FETCH_ASSOC)) 
		{
			...
			echo "<option value=\"".$fila1["id_tipo"]."\"".$selected.">".utf8_decode($fila1["nombre"])."</option>\n";
		}
