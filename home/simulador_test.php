...
while ($fila1 = mysqli_fetch_assoc($parametros))
{
...
$fila1 = mysqli_fetch_assoc($maxcd);
...
if ($_REQUEST["cedula"])
{
	...
	if ((mysqli_num_rows($consultas_realizadas) < $maxcd) || (!$_SESSION["FUNC_MAXCONSDIARIAS"]))
	{
		if (!$_REQUEST["pagad"])
		{
			...
			{
				$fila = mysqli_fetch_assoc($empleado);
		...
		if (mysqli_num_rows($empleado))
		{
			...
			if (!mysqli_num_rows($existe_simulacion) || $omitir_validacion_30_dias)
			{
				...
				$fila_salario_minimo = mysqli_fetch_assoc($rs_salario_minimo);
				...
				else
				{
					...
					{
						$fila_tasa = mysqli_fetch_assoc($rs_tasa);
						...
						{
							$fila_tasa2 = mysqli_fetch_assoc($rs_tasa2);
				...
				if ($_SESSION["FUNC_MUESTRACAMPOS2"])
				{
					...
					$fila_adicionales = mysqli_fetch_assoc($rs_adicionales);
				...
				if ($_SESSION["FUNC_MUESTRACAMPOS2"] && strtoupper($fila["ciudad_labora"]) == "VALLEDUPAR")
				{
					...
					$fila_bonificacion = mysqli_fetch_assoc($rs_bonificacion);
				...
				while ($fila1 = mysqli_fetch_assoc($embargos))
				{
					if (!$fila1["fechafin"])
					{
						$embargo_actual = "SI";
				...
				while ($fila1 = mysqli_fetch_assoc($descuentos))
				{
...
if ($_REQUEST["id_simulacion"])
{
	...
	{
		$fila = mysqli_fetch_assoc($simulacion);
		...
		while ($fila2 = mysqli_fetch_assoc($rs2))
		{
			...
			if ($entidad[$fila2["consecutivo"] - 1])
			{
				...
				{
					$fila1 = mysqli_fetch_assoc($entidad_tmp);
		...
		while($fila1 = mysqli_fetch_assoc($agenda))
		{
...
$query_oficina = mysqli_query($link, "SELECT IF(b.ado IS NULL, 0, b.ado) AS ado, IF(b.gattaca IS NULL, 0, b.gattaca) AS gattaca  FROM oficinas_usuarios a JOIN oficinas b ON a.id_oficina = b.id_oficina WHERE a.id_usuario = '".$id_comercial."' LIMIT 1");
if(mysqli_num_rows($query_oficina) > 0){
	$datos_ofi = mysqli_fetch_assoc($query_oficina);
...
<script language="JavaScript">
	...
	function soloCaracterPermitido(string){
	    var out = '';
	    var filtro = 'Aa Eee Ii Oo Uu Bb Cc Dd Ff Gg Hh Jj Kk Ll Mm Nn Ñn Pp Qq Rr Ss Tt Vv Ww Xx Yy Zz0123456789%#$?¿!¡*-+.,:=';//Caracteres validos
	    ...
	    for (var i=0; i<string.length; i++){
	       	if (filtro.indexOf(string.charAt(i)) != -1) {
		        ...
		        if((ascii == 193 || ascii == 201 || ascii == 211 || ascii == 205 || ascii == 218)){
...
<?php
	...
	while ($fila2 = mysqli_fetch_assoc($datos_subestados_viable))
	{
	...
	while ($fila2 = mysqli_fetch_assoc($datos_subestados_negado))
	{
		$padre_hija .= "\"".$fila2["id_subestado"]."-".$fila2["nombre"]."\",";
	}
...
<?php
	...
	while ($fila2 = mysqli_fetch_assoc($rs_tasas))
	{
...
<?php
			...
			while ($fila3 = mysqli_fetch_assoc($rs_tasas2))
			{
...
<form name="formato" method="post" action="simulador2_test.php" onSubmit="return chequeo_forma()">
		...
		<tr>
			...
			<td>
				<select id="tipo_tasa_comision" name="tipo_tasa_comision"  style="width:195px; background-color:#EAF1DD;">
					...
					echo($queryTasaC);
					while ($fTC = mysqli_fetch_assoc($rsTC)){ 
...
?>
		<table border="0" cellspacing=1 cellpadding=2 width="95%">
		<tr>
			<td colspan="2"><textarea name="observaciones" onkeyup="this.value=validarSoloCaracterPermitido(this.value)" rows="3" style="width:100%; background-color:#EAF1DD;"><?php echo $observaciones ?></textarea></td>
		</tr>
		</table>
<?php
...
?>
		...
		<iframe id="frm_observaciones" src="simulaciones_observaciones_test.php?id_simulacion=<?php echo $_REQUEST["id_simulacion"] ?>&tipo=<?php echo $_REQUEST["tipo"] ?>" width="100%" height="300px" seamless style="overflow:auto"></iframe>
<?php
...
?>
		...
		{
			$resFormularioDigital=mysqli_fetch_assoc($queryFormularioDigital);
			?>
...
<!--				<tr>
				...
				<tr>
					...
					<td><input type="text" name="clave" value="<?php echo $clave ?>" size="25" style="text-align:center;<?php if ($_SESSION["S_TIPO"] == "ADMINISTRADOR" || $_SESSION["S_TIPO"] == "OPERACIONES" || $_SESSION["S_SUBTIPO"] == "ANALISTA_CREDITO" || $_SESSION["S_SUBTIPO"] == "COORD_CREDITO") { ?> background-color:#EAF1DD"<?php } else { ?>" readonly<?php } ?>></td>
				</tr>
				</table>
			</td>
			<td>
					        	...
					        	<div class="box1 oran clearfix">
					        		<table border="1" class="table" >
					        			<tr align="center" style="text-align: center;">
					        				<td align="center">
					        					<div id="consulta_cifinUP">
					        						<img src="../images/antecedentes.png" style="width: 70px; height: 18px;">
					        						<h3>Antecedentes</h3>
					        					</div>
					        					<div class="badges">
					        						<div class="badge-success" style="display: none;" id="disponible-gattacaAJ" type="button" servicio="ANTECEDENTES"
					        						proveedor="GATTACA">
						        						<img src="../images/chequeado.png" style="margin-top: inherit;">
						        					</div>
						        					<div class="badge-info" style="display: none;"  id="nodisponible-gattacaAJ" servicio="ANTECEDENTES"
						        					proveedor="GATTACA">
							        					<img src="../images/novalidado.png" style="margin-top: inherit;">
							        				</div>
							        				<div class="badge-error" id="calendario-gattacaAJ" servicio="ANTECEDENTES"
							        				proveedor="GATTACA" data-service="consulta_ws">
								        				<img src="../images/reloj_arena.png" style="margin-top: inherit;">
								        			</div>
					        					</div>
					        					<label id="label-gattacaAJ">Generado</label>
					        				</td>			        	
										</tr>
									</table>
									<br>
...
<tr>
	<td valign="top">
		...
		$val=0;
		//$queryAnalistasKreditPlus=mysqli_query("SELECT id_usuario FROM usuarios a WHERE id_usuario NOT IN (SELECT descripcion FROM definicion_tipos WHERE id_tipo=4) AND subtipo='ANALISTA_CREDITO' AND estado=1 AND id_usuario='".$_SESSION["S_IDUSUARIO"]."'",$link);
			...
			<select name="id_analista_riesgo_operativo" style="background-color:#EAF1DD; width:150px;">
					...
					<?php
						...
						if (mysqli_num_rows($queryOficinaNegocioEspecial)>0)
						{
							...
							FROM 
							usuarios a 
							WHERE a.id_usuario IN (SELECT descripcion FROM definicion_tipos WHERE id_tipo=4) AND (a.subtipo='ANALISTA_CREDITO' OR a.subtipo='NEXA') and a.disponible in ('s','g') and a.estado=1)";

						}else{
							$consultarUsuariosInicio="(SELECT a.* 
							FROM 
							usuarios a 
							WHERE a.id_usuario not IN (SELECT descripcion FROM definicion_tipos WHERE id_tipo=4) AND (a.subtipo='ANALISTA_CREDITO' OR a.subtipo='NEXA') and a.disponible in ('s','g') and a.estado=1)";
						}
...
?>
		<table border="0" cellspacing=1 cellpadding=2 width="95%">
		<tr>
			<td colspan="4"><textarea name="observaciones" onkeyup="this.value=validarSoloCaracterPermitido(this.value)" rows="3" style="width:100%; background-color:#EAF1DD;"><?php echo $observaciones ?></textarea></td>
		</tr>
		</table>
<?php
...
?>
		<input type="hidden" name="observaciones" value="">
		<iframe id="frm_observaciones" src="simulaciones_observaciones_test.php?id_simulacion=<?php echo $_REQUEST["id_simulacion"] ?>&tipo=<?php echo $_REQUEST["tipo"] ?>" width="100%" height="300px"></iframe>
<?php
