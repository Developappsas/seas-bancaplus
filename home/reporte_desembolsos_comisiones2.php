...
INNER JOIN oficinas ofi ON si.id_oficina = ofi.id_oficina LEFT JOIN solicitud so ON si.id_simulacion = so.id_simulacion LEFT JOIN ciudades ci ON so.ciudad = ci.cod_municipio LEFT JOIN caracteristicas ca ON si.id_caracteristica = ca.id_caracteristica 
...
while ($fila = sqlsrv_fetch_array ($rs))
{
	...
	{
		sqlsrv_fetch($descuentos_adicionales,SQLSRV_SCROLL_FIRST);
