...
curl_setopt_array($curl, array(
  ...
  CURLOPT_POSTFIELDS =>'{"id_simulacion":"'.$_REQUEST["id_simulacion"].'","origen":"TESORERIA","usuario":"'.$_SESSION["S_IDUSUARIO"].'","operacion":"Crear Simulaciones Consultas Log"}',
...
$queryDB = "SELECT CONCAT(us.nombre,' ',us.apellido) as nombre_comercial,si.*, FORMAT(fecha_comision_pagada, 'Y-m-d') as fecha_comision_pagada_texto, ba.nombre as nombre_banco, FORMAT(si.fecha_cartera, 'yyyy-MM') as mes_cartera, so.direccion, so.email as mail, pa.sector 
from simulaciones si 
	...
	LEFT JOIN usuarios us ON us.id_usuario=si.id_comercial 
...
$rs1 = sqlsrv_query($link,"select SUM(valor_girar) as s from giros where id_simulacion = '".$_REQUEST["id_simulacion"]."' and clasificacion = 'DSC' and fecha_giro IS NOT NULL");
...
$rs1 = sqlsrv_query($link,"select SUM(valor_girar) as s from giros where id_simulacion = '".$_REQUEST["id_simulacion"]."'");
...
if (fecha_giro<?php echo $fila1["id_giro"] ?>.value != "") {
		...
		if(nro_cheque<?php echo $fila1["id_giro"] ?>.value != "" || id_cuentabancaria<?php echo $fila1["id_giro"] ?>.value != ""){
			alert("Debe establecer la fecha o los demás campos del giro de la compra de cartera (<?php echo str_replace("\"", "", utf8_decode($fila1["beneficiario"])) ?>)");
			return false;
		}
	}
<?php
...
<tr>
	...
	<td class="titulo"><center><b>Detalle Tesorer&iacute;a 1</b><br><br></center></td>
</tr>
</table>
...
<input type="hidden" name="valor_por_millon_seguro_parcial" value="<?php echo $fila["valor_por_millon_seguro_parcial"] ?>">
<input type="hidden" name="valor_por_millon_seguro_base" value="<?php echo $fila["valor_por_millon_seguro_base"] ?>">
...
<tr>
	<td valign="top">
		...
		<tr>
			<td>ESTADO</td>
			<td><input style="background: yellow;"  type="text" name="estado" value="<?php echo $estado ?>" size="45" readonly></td>
		</tr>
		<tr>
			<td>FECHA</td>
	...
	<td valign="top">
		...
		<tr>
			...
			<td><input type="text" name="asesoria_financiera" value="<?php echo number_format($asesoria_financiera, 0, ".", ",") ?>" size="15" style="text-align:right;" readonly></td>
		</tr>
		<tr>
			<td>- IVA</td>
			<td><input type="text" name="iva" value="<?php echo number_format($iva, 0, ".", ",") ?>" size="15" style="text-align:right;" readonly></td>
		</tr>
		<tr>
			<td>- SERVICIO EN LA NUBE</td>
			<td><input type="text" name="servicio_nube" value="<?php echo number_format($servicio_nube, 0, ".", ",") ?>" size="15" style="text-align:right;" readonly></td>
		</tr>
		<tr>
			<td>- GMF</td>
...
?>
		...
		<?php
			...
			$queryFacturado=sqlsrv_query($link,$consultarFacturado);
			$FACTURADO="NO";
		...
		<td>FACTURADO</td>
			<td>
			<input type="text" name="facturado_texto" value="<?php echo $rptaFacturado;?>" size="15" style="text-align:center;" readonly>
				<?php if ($_SESSION["S_TIPO"] == "ADMINISTRADOR" || $_SESSION["S_TIPO"] == "CONTABILIDAD") 
				{ ?>
				 <br>
				 <a href="#" id="verHistorialFacturado" name="<?php echo $fila["id_simulacion"] ?>">Ver Historial</a></td>
				 <?php
				}
				 ?>
		</tr>
...
<tr>
	...
	<?php if ($_SESSION["FUNC_ADJUNTOS"]) { ?><th><img src="../images/archivos.png" title="Cert. Modif"></th><?php } ?>
...
while ($fila2 = sqlsrv_fetch_array($rs2, SQLSRV_FETCH_ASSOC))
{
	...
	{
		$agenda_tmp = sqlsrv_query($link,"SELECT fecha_vencimiento from agenda where id_simulacion = '".$_REQUEST["id_simulacion"]."' AND consecutivo = '".$fila2["consecutivo"]."'", array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
		...
		$cc_tmp = sqlsrv_query($link,"SELECT tcc.consecutivo,tcc.pagada, tcc.cuota_retenida, tcc.fecha_giro as fecha_girocc, ad.nombre_grabado, tcc.id_adjunto, tcc.usuario_firma_cheque, tcc.fecha_firma_cheque from tesoreria_cc tcc LEFT JOIN adjuntos ad ON tcc.id_adjunto = ad.id_adjunto where tcc.id_simulacion = '".$_REQUEST["id_simulacion"]."' AND tcc.consecutivo = '".$fila2["consecutivo"]."'", array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
...
<?php if (($_SESSION["S_TIPO"] == "ADMINISTRADOR" || $_SESSION["S_TIPO"] == "TESORERIA") && $_SESSION["S_SOLOLECTURA"] != "1") { ?><td align="center"><input type="checkbox" name="pagada<?php echo $fila2["consecutivo"] ?>" id="pagada<?php echo $fila2["consecutivo"] ?>" value="1"<?php if ($pagada) { ?> checked<?php } ?>></td><?php } else { ?><input type="hidden" name="pagada<?php echo $fila2["consecutivo"] ?>" value="<?php echo $pagada ?>"><?php } ?>
	...
	<?php if ($_SESSION["FUNC_ADJUNTOS"]) { ?><td align="center"><?php if ($fila2["nombre_grabado"]) { ?><a onClick="window.open($urlPrincipal.'/plugins/prueba.php?id_simulacion=<?php echo $_REQUEST["id_simulacion"];?>&consecutivo=<?php echo $fila2["consecutivo"];?>','CERTIFICACION', 'toolbars=yes,scrollbars=yes,resizable=yes,width=800,height=600,top=0,left=0');"><img src="../images/archivos.png" title="Certificaci&oacute;n"></a><?php } else { echo "&nbsp;"; } ?></td><?php } ?>
...
<tr>
<?php if (($_SESSION["S_TIPO"] == "ADMINISTRADOR" || $_SESSION["S_TIPO"] == "TESORERIA" || (($_SESSION["S_SUBTIPO"] == "ANALISTA_TESORERIA" || $_SESSION["S_SUBTIPO"] == "ANALISTA_JURIDICO") && !$giros_realizados)) && ($fila["estado_tesoreria"] != "CER") && ($_SESSION["S_SOLOLECTURA"] != "1") && !$inconsistencia) {?><th>Estado</th><?php } ?>
	<th>Beneficiario</th>
	<th>Nit</th>
	<th>Forma Pago</th>
	<th>Valor a Girar</th>
...
<?php if (($_SESSION["S_TIPO"] == "ADMINISTRADOR" || $_SESSION["S_TIPO"] == "TESORERIA" || (($_SESSION["S_SUBTIPO"] == "ANALISTA_TESORERIA" || $_SESSION["S_SUBTIPO"] == "ANALISTA_JURIDICO") && !$giros_realizados)) && ($fila["estado_tesoreria"] != "CER") && ($_SESSION["S_SOLOLECTURA"] != "1") && !$inconsistencia) { ?>
	<td>
		<?php if($fila1["estado"] == 1){ ?>
			<input type="button" style="background: #1ba101 !important;" value="APROBADO">
		<?php }else{ ?>
			<input type="button" style="background: #ff5200 !important; cursor: pointer;" value="PENDIENTE" onclick="procesarGiro(this, '<?php echo $fila1["id_giro"] ?>', '<?=$_REQUEST["id_simulacion"]?>', '<?=$clasificacion." / ".str_replace("\"", "&#34;", utf8_decode($fila1["beneficiario"]))?>')">
		<?php } ?>
	</td>
	<?php } ?>
...
<div class="modal" id="modalHistorialFacturacion" data-animation="slideInOutLeft">
	<div class="modal-dialog">
    	<header class="modal-header">
        	Historial Pago Comisiones
        	<button type="button" class="close-modal" data-close>x</button>
		...
		<section class="modal-content">
			<div id="divTablaHistorialFacturacion">
				<table id="tablaHistorialFacturacion">
				</table>
			</div>
        </section>
        <footer class="modal-footer">
...
$('#verHistorialFacturado').click(function(e){
		cargarHistorialFacturado($('#verHistorialFacturado').attr("name"));
	});

	function cargarHistorialFacturado(id_simulacion){
		$.ajax({
			type: 'POST',
			url: '../servicios/tesoreria/consultar_historial_facturacion_credito.php',
			data: "exe=consultarHistorialFacturado&id_simulacion="+id_simulacion,
			cache: false,
			...
			success: function(data) {
				...
				Swal.showLoading();
				$('#tablaHistorialFacturacion').DataTable( {
					scrollX: true,
					...
					"pageLength": 40,
					"columns": [
						{ title: 'Facturado', mData: 'facturado', orderable: false},
		...
		$("#modalHistorialFacturacion").addClass('is-visible');
	}
	...
	function procesarGiro(elemet, id_giro, id_simulacion, descripcion) {

		Swal.fire({
	        title: descripcion,
	        html: '<p style="text-align: justify;">Para cambiar el estado del giro, pulse en los botones: <br><br><b style="color: #1ba101;">1. APROBAR: </b> el sistema validará los saldos para cerrar o no el credito.<br> <b style="color: #ff5200;">2. RECHAZAR:</b> El sistema eliminará el giro y pasará el creditó al subestado 6.1 para ser validado.<p>',
	        showDenyButton: true,
	        showCancelButton: true,
	        confirmButtonText: 'APROBAR',
	        denyButtonText: 'RECHAZAR',
	        cancelButtonText: 'CANCELAR',
	        confirmButtonColor: '#1ba101',
	        denyButtonColor: '#ff5200',
	    }).then((result) => {        
	        if (result.isConfirmed) {
	        	Swal.fire({
					title: '¿Está seguro de APROBAR el GIRO?',
					text: "Una vez aprobado el giro, el sistema validará los saldos para cerrar o no el credito.",
					icon: 'question',
					showCancelButton: true,
	        		confirmButtonColor: '#1ba101',
	        		cancelButtonText: 'CANCELAR',
					...
					if (result.isConfirmed) {
						$.ajax({
							url: '../servicios/tesoreria/aprobar_giro.php',
							type: 'POST',
							data: { id_giro : id_giro, id_simulacion : id_simulacion },
							dataType : 'json',
							success: function(json) {
								...
								if(json.code == 200){
									Swal.fire(
										'Giro Aprobado',
										'El giro se ha Aprobado Exitosamente',
										'success'
									);	
								}else{
									if(json.mensaje && json.mensaje != ''){
										Swal.fire(
											'Error!',
											json.mensaje,
											'error'
	            ...
	            Swal.fire({
					title: '¿Está seguro de RECHAZAR el GIRO?',
					text: "Una vez rechazado el giro, el sistema eliminará el giro y pasará el creditó al subestado 6.1 para ser validado.",
					icon: 'question',
					showCancelButton: true,
					...
					if (result.isConfirmed) {
						$.ajax({
							url: '../servicios/tesoreria/rechazar_giro.php',
							type: 'POST',
							data: { id_giro : id_giro, id_simulacion : id_simulacion },
							dataType : 'json',
							success: function(json) {
								...
								if(json.code == 200){
									Swal.fire(
										'Giro Rechazado',
										'El giro se ha Eliminado Exitosamente',
										'success'
									)	
								}else{
									if(json.mensaje && json.mensaje != ''){
